...
def del_sd_config():
    ...
    try:
        ...
        if data != None:
            ...
            word_to_remove_kind = ["api"]
            worker_kind_new = '|'.join(word for word in worker_kind.split('|') if word not in word_to_remove_kind)

            worker_dist = data['worker_dist']
            word_to_remove_dist = ["sd_SR","sd_webui","sd_anime","sd_sketch"]
            worker_dist_new = '|'.join(word for word in worker_dist.split('|') if word not in word_to_remove_dist)

            data['worker_kind'] = worker_kind_new
            data['worker_dist'] = worker_dist_new

            BASE_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
            path = os.path.join(BASE_DIR, 'worker.config')
            with open(path, 'w') as file:
                json.dump(data, file, indent=4)
                ret = True
    except Exception as e:
        print('fix_config exception: {}'.format(e))
    return ret
...
def add_sd_config():
    ...
    try:
        ...
        if data != None:
            ...
            worker_kind_new = worker_kind + "|api"
            worker_dist_new = worker_dist + "|sd_SR|sd_webui|sd_anime|sd_sketch"
...
def get_config_data():
    ...
    try:
        ...
        print('get_config_data exception: {}'.format(e))
    return data     
...
def registerWorker():
    ...
    try:
        ...
        if data != None:
            ...
            jobserv_host = data['job_server']
            url = jobserv_host + "/api/worker/add?name=" + data['worker_name'] \
                + "&category=" + data['worker_category'] \
                + "&property=" + data['worker_property'] \
                + "&host=" + data['worker_host'] \
                + "&port=" + data['worker_port'] \
                + "&kind=" + data['worker_kind'] \
                + "&dist=" + data['worker_dist']\
                + "&edition=" + data['worker_edition'] \
                + "&ver=" + formatted_date
            res = requests.get(url)
            if res.ok:
                res = res.json()
                if res['code'] == 200:
                    glob.set_value('worker_id', str(res['data']))
                    ret = True
    except Exception as e:
        print('registerWorker exception: {}'.format(e))
    return ret

def report_feishu(str_message):
    ret = False
    try:
        data = get_config_data()
        if data != None:
            ...
            response_feishu = requests.post(url, data = json.dumps(body), headers = header)
            ret = True
    except Exception as e:
        print('report_feishu exception: {}'.format(e))
    return ret
    ...
    def img2img(
        ...
        for i in range(3):
            try:
                ...
                if i == 2:
                    print("{}/img2img: failed".format(self.baseurl))
                    raise e
                time.sleep(1)
    ...
    def interrogate(self, image):
        for i in range(3):
            try:
                ...
                if i == 2:
                    print("{}/interrogate: failed".format(self.baseurl))
                    raise e
                time.sleep(1)
    ...
    def set_options(self, options):
        for i in range(3):
            try:
                ...
                if i == 2:
                    print("{}/options: failed".format(self.baseurl))
                    raise e
                time.sleep(1)
